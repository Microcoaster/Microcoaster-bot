...
module.exports = {
  execute: async (interaction) => {
    ...
    try {
      ...
      const newValue = interaction.fields.getTextInputValue('config_value').trim();
      ...
      const validationResult = await validateConfigValue(interaction, fieldKey, newValue, category);
      ...
      await applyConfigChanges(interaction.client, fieldKey, validationResult.value);

      // Créer l'embed de confirmation
      const successEmbed = new EmbedBuilder()
        .setColor('#00ff00')
        .setTitle('✅ Configuration Updated')
        .setDescription(`Successfully updated **${getFieldDisplayName(fieldKey)}**`)
        .addFields(
          { name: 'Previous Value', value: formatConfigValue(oldValue, fieldKey, interaction.guild), inline: true },
          { name: 'New Value', value: formatConfigValue(validationResult.value, fieldKey, interaction.guild), inline: true },
      ...
      await logConfigChange(interaction.client, config, category, fieldKey, oldValue, validationResult.value, interaction.user);
...
async function validateConfigValue(interaction, fieldKey, value) {
  ...
  switch (fieldType) {
    case 'role': {
      ...
      if (!role) {
        return { valid: false, error: `Role with ID \`${value}\` not found in this server.` };
      }
    ...
    case 'channel': {
      ...
      if (!channel) {
        return { valid: false, error: `Channel with ID \`${value}\` not found in this server.` };
      }
      return { valid: true, value: value };
    }
    ...
    case 'category': {
      ...
      if (!category || category.type !== 4) { // 4 = CategoryChannel
        return { valid: false, error: `Category with ID \`${value}\` not found in this server.` };
      }
      return { valid: true, value: value };
    }
    ...
    case 'number': {
      ...
      if (fieldKey === 'default_warranty_duration_months' && (num < 1 || num > 120)) {
        return { valid: false, error: 'Warranty duration must be between 1 and 120 months.' };
      }
      if (fieldKey === 'max_activation_attempts' && (num < 1 || num > 10)) {
        return { valid: false, error: 'Max activation attempts must be between 1 and 10.' };
      }
      if (fieldKey === 'cleanup_interval_hours' && (num < 1 || num > 168)) {
        return { valid: false, error: 'Cleanup interval must be between 1 and 168 hours (1 week).' };
      }
      if (fieldKey === 'ticket_auto_close_after_hours' && (num < 1 || num > 720)) {
        return { valid: false, error: 'Auto close time must be between 1 and 720 hours (30 days).' };
      }
      if (fieldKey === 'max_tickets_per_user' && (num < 1 || num > 10)) {
        return { valid: false, error: 'Max tickets per user must be between 1 and 10.' };
      }
    ...
    case 'text':
      ...
      if (value.length > 100) {
        return { valid: false, error: 'Text value must be less than 100 characters.' };
      }
      return { valid: true, value: value };
  ...
  try {
    switch (fieldKey) {
      ...
      case 'bot_activity_type': {
        ...
        await client.user.setActivity(config.bot_status || 'MicroCoaster™ Support', { type: newValue });
        break;
      }
    }
  } catch (error) {
    console.error('Erreur lors de l\'application des changements de configuration:', error);
  }
}
...
function formatConfigValue(value, fieldKey, guild) {
  ...
  switch (fieldType) {
    case 'role': {
      ...
      return role ? `${role.name} (\`${role.id}\`)` : `Unknown Role (\`${value}\`)`;
    }
      
    case 'channel': {
      const channel = guild.channels.cache.get(value);
      return channel ? `${channel.name} (\`${channel.id}\`)` : `Unknown Channel (\`${value}\`)`;
    }
      
    case 'category': {
      const category = guild.channels.cache.get(value);
      return category ? `${category.name} (\`${category.id}\`)` : `Unknown Category (\`${value}\`)`;
    }
 ...
 */
async function logConfigChange(client, config, category, fieldKey, oldValue, newValue, user) {
  try {
    const statsChannelId = config.channels ? config.channels.bot_stats_channel_id : config.bot_stats_channel_id;
    ...
    const logEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Previous Value', value: formatConfigValue(oldValue, fieldKey, logChannel.guild), inline: false },
        { name: 'New Value', value: formatConfigValue(newValue, fieldKey, logChannel.guild), inline: false }
      )
      .setTimestamp();

    await logChannel.send({ embeds: [logEmbed] });
  } catch (error) {
    console.error('Erreur lors de l\'enregistrement du log de configuration:', error);
  }
}
...
function getNestedConfigValue(config, category, fieldKey) {
  if (category && config[category] && config[category][fieldKey] !== undefined) {
