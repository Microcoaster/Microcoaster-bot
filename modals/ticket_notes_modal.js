...
module.exports = {
  execute: async (interaction) => {
    try {
      ...
      const internalNotes = interaction.fields.getTextInputValue('internal_notes');
      const noteCategory = interaction.fields.getTextInputValue('note_category') || 'General';
      ...
      const notesEmbed = new EmbedBuilder()
        ...
        .setFooter({ text: `Ticket ID: ${ticketId} • Internal Staff Communication` })
        .setTimestamp();
      ...
      console.log(`\x1b[38;5;5m📋 Notes internes ajoutées au ticket ${ticketId} par ${user.username}\x1b[0m`);

    } catch (error) {
      console.error('Erreur lors de l\'ajout de notes internes:', error);
      
      if (interaction.deferred) {
        await interaction.editReply({ content: '❌ An error occurred while adding internal notes.' });
      } else {
        await interaction.reply({ content: '❌ An error occurred while adding internal notes.', flags: MessageFlags.Ephemeral });
      }
    }
  }
};
