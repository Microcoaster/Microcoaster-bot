...
class TicketDAO {
  ...
  async getConnection() {
    try {
      ...
      console.error(
        `\x1b[38;5;1m❌ Erreur de connexion MySQL: ${error.message}\x1b[0m`,
      );
      throw error;
    }
  }
  ...
  async getNextTicketNumber() {
    try {
      ...
      console.error(
        `\x1b[38;5;3m⚠️ Impossible d'accéder à la base de données, utilisation d'un numéro temporaire\x1b[0m`,
      );
  ...
  async createTicket(
    ...
    try {
      ...
      console.error(
        `\x1b[38;5;3m⚠️ Impossible de sauvegarder le ticket en base de données: ${error.message}\x1b[0m`,
      );
      // Retourner des données simulées pour que le ticket fonctionne
      const fallbackNumber =
        ticketNumber || Math.floor(Date.now() / 1000) % 10000;
      return {
        id: fallbackNumber,
  ...
  async getUserActiveTicket(userId, guildId, ticketType = null) {
    try {
      const connection = await this.getConnection();
      let query =
        'SELECT * FROM support_tickets WHERE user_id = ? AND status != "closed"';
      let params = [userId];
      ...
      console.error(
        `\x1b[38;5;3m⚠️ Impossible de vérifier les tickets existants: ${error.message}\x1b[0m`,
      );
  ...
  async createTranscription(ticketId, content, createdBy) {
    ...
    try {
      ...
      console.log(
        `\x1b[38;5;2m✅ Transcription créée pour le ticket ${ticketId}\x1b[0m`,
      );
      return true;
    } catch (error) {
      console.error(
        `\x1b[38;5;1m❌ Erreur lors de la création de la transcription: ${error.message}\x1b[0m`,
      );
      return false;
    }
  }
  ...
  async deleteTicket(ticketId) {
    ...
    try {
      ...
      console.log(
        `\x1b[38;5;3m🗑️  Ticket ${ticketId} supprimé de la base de données\x1b[0m`,
      );
      return result.affectedRows > 0;
    } catch (error) {
      console.error(
        `\x1b[38;5;1m❌ Erreur lors de la suppression du ticket ${ticketId}: ${error.message}\x1b[0m`,
      );
      return false;
    }
  }
  ...
  async deleteTicketByChannelId(channelId) {
    ...
    try {
      ...
      console.error(
        `\x1b[38;5;1m❌ Erreur lors de la suppression du ticket par channel ID ${channelId}: ${error.message}\x1b[0m`,
      );
      return false;
    }
  }
  ...
  async getAllOpenTicketsWithChannels() {
    ...
    try {
      ...
      console.error(
        `\x1b[38;5;1m❌ Erreur lors de la récupération des tickets ouverts: ${error.message}\x1b[0m`,
      );
      return [];
    }
  }
   ...
   */
  async closeTicketInstantly(
    ticketId,
    channelId,
    guild,
    reason = "Fermé par un administrateur",
  ) {
    try {
      ...
      if (channelId && guild) {
        ...
        if (channel) {
          ...
          console.log(
            `\x1b[38;5;2m✅ Ticket ${ticketId} fermé instantanément et salon supprimé\x1b[0m`,
          );
        } else {
          console.log(
            `\x1b[38;5;3m⚠️  Salon ${channelId} du ticket ${ticketId} déjà supprimé\x1b[0m`,
          );
        }
      }
      ...
      console.error(
        `\x1b[38;5;1m❌ Erreur lors de la fermeture instantanée du ticket ${ticketId}: ${error.message}\x1b[0m`,
      );
      return false;
    }
  }
