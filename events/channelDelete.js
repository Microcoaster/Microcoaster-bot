...
module.exports = {
    ...
    async execute(channel) {
        try {
            ...
            if (ticket) {
                console.log(`\x1b[38;5;3m🗑️  Salon de ticket supprimé détecté: ${channel.name} (${channel.id})\x1b[0m`);
                console.log(`\x1b[38;5;3m📋  Ticket associé: ${ticket.ticket_id}\x1b[0m`);
                
                // Supprimer le ticket de la base de données
                const deleted = await ticketDAO.deleteTicketByChannelId(channel.id);
                
                if (deleted) {
                    console.log(`\x1b[38;5;2m✅ Ticket ${ticket.ticket_id} supprimé de la base de données\x1b[0m`);
                } else {
                    console.log(`\x1b[38;5;1m❌ Échec de la suppression du ticket ${ticket.ticket_id} de la base de données\x1b[0m`);
                }
            }
        } catch (error) {
            console.error(`\x1b[38;5;1m❌ Erreur lors de la gestion de la suppression du salon ${channel.id}: ${error.message}\x1b[0m`);
        }
    }
};
