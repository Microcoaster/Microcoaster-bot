...
pool.on('connection', function (connection) {
  console.log('\x1b[38;5;6m🔗  Nouvelle connexion MySQL établie (ID: ' + connection.threadId + ')\x1b[0m');
});
  ...
  try {
    ...
    if (sharedConnection) {
      try {
        ...
        console.log('\x1b[38;5;3m🔄 Connexion expiree, creation d\'une nouvelle...\x1b[0m');
        sharedConnection = null;
      }
    }

    // Créer une nouvelle connexion
    console.log('\x1b[38;5;4m🔗 Creation d\'une nouvelle connexion partagee...\x1b[0m');
    ...
    console.error(`\x1b[38;5;1m❌ Erreur lors de la creation de la connexion partagee: ${error.message}\x1b[0m`);
    throw error;
  }
}
...
async function initializeDatabase() {
  ...
  try {
    ...
    console.log("\x1b[38;5;4m🔗  Test de connexion à la base de données...\x1b[0m");
    ...
    try {
      ...
      console.log(`\x1b[38;5;2m✅  Connexion établie en ${connectionTime}ms\x1b[0m`);
    } catch (connectionError) {
      console.error(`\x1b[38;5;1m❌  Échec de connexion: ${connectionError.message}\x1b[0m`);
      console.log(`\x1b[38;5;3m⚠️  Le bot va démarrer en mode sans base de données\x1b[0m`);
    ...
    console.log(`\x1b[38;5;4m🔧  Exécution de ${sqlStatements.length} requêtes SQL...\x1b[0m`);
    console.log(`\x1b[38;5;3m⏱️   Note: Connexion distante, les requêtes peuvent prendre du temps...\x1b[0m`);
    ...
    for (let i = 0; i < sqlStatements.length; i++) {
      ...
      if (statement) {
        try {
          ...
          console.log(`\x1b[38;5;4m🔄  Exécution requête ${i + 1}/${sqlStatements.length}: ${finalStatement.substring(0, 60)}...\x1b[0m`);
          
          const queryStart = Date.now();
          await connection.execute(finalStatement);
          const queryTime = Date.now() - queryStart;
          
          console.log(`\x1b[38;5;2m✅  Requête ${i + 1} exécutée en ${queryTime}ms\x1b[0m`);
        } catch (sqlError) {
          console.error(`\x1b[38;5;1m❌  Erreur requête ${i + 1}:\x1b[0m`);
          console.error(`\x1b[38;5;1m   SQL: ${statement.substring(0, 100)}...\x1b[0m`);
          ...
          if (sqlError.message.includes('already exists') || 
            ...
            console.log(`\x1b[38;5;3m⚠️   Élément déjà existant, on continue...\x1b[0m`);
            continue;
          } else {
            throw sqlError;
          }
    ...
    console.log(`\x1b[38;5;4m🔍  Vérification des tables créées...\x1b[0m`);
    const [tables] = await connection.execute('SHOW TABLES');
    
    console.log(`\x1b[38;5;2m📊  ${tables.length} table(s) présente(s) dans la base :\x1b[0m`);
    tables.forEach(table => {
      const tableName = Object.values(table)[0];
      console.log(`\x1b[38;5;2m   • ${tableName}\x1b[0m`);
    });
    
    // Vérifier spécifiquement les tables importantes
    const expectedTables = [
      'warranty_premium_codes',
      'user_roles_backup', 
      'support_tickets',
      'moderation_logs',
      'user_points',
      'role_restoration_logs',
      'warranty_activation_logs',
      'ticket_counter'
    ];
    
    const existingTableNames = tables.map(table => Object.values(table)[0]);
    
    console.log(`\x1b[38;5;4m📋  Vérification des tables essentielles...\x1b[0m`);
    for (const expectedTable of expectedTables) {
      if (existingTableNames.includes(expectedTable)) {
        console.log(`\x1b[38;5;2m   ✅ ${expectedTable}\x1b[0m`);
      } else {
        console.log(`\x1b[38;5;1m   ❌ ${expectedTable} MANQUANTE\x1b[0m`);
      }
    }
