...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("setup-bot")
    .setDescription(
      "Complete bot setup: creates all roles, channels, categories and configures the entire system",
    )
  ...
  async execute(interaction) {
    try {
      ...
      const setupEmbed = new EmbedBuilder()
        ...
        .setDescription(
          "Creating roles, channels, and categories. This may take a moment...",
        )
        .setTimestamp();
      ...
      for (const [roleKey, roleConfig] of Object.entries(rolesToCreate)) {
        try {
          ...
          if (existingRole && !overwrite) {
            ...
            createdElements.roles[roleKey] = {
              name: roleConfig.name,
              id: existingRole.id,
              status: "existing",
            };
            console.log(
              `Role ${roleConfig.name} already exists, using existing role`,
            );
          } else {
            if (existingRole && overwrite) {
              await existingRole.delete("Recreating role during bot setup");
              console.log(
                `Deleted existing role ${roleConfig.name} for recreation`,
              );
            }
          ...
          createdElements.errors.push(
            `Failed to create role ${roleConfig.name}: ${error.message}`,
          );
        }
      }
      ...
      for (const [categoryKey, categoryConfig] of Object.entries(
        ...
        try {
          ...
          if (existingCategory && !overwrite) {
            ...
            createdElements.categories[categoryKey] = {
              name: categoryConfig.name,
              id: existingCategory.id,
              status: "existing",
            };
            console.log(
              `Category ${categoryConfig.name} already exists, using existing category`,
            );
          } else {
            if (existingCategory && overwrite) {
              await existingCategory.delete(
                "Recreating category during bot setup",
              );
              console.log(
                `Deleted existing category ${categoryConfig.name} for recreation`,
              );
            }
            ...
            console.log(
              `Created category: ${categoryConfig.name} (${category.id})`,
            );
          }
        } catch (error) {
          console.error(
          ...
          createdElements.errors.push(
            `Failed to create category ${categoryConfig.name}: ${error.message}`,
          );
        }
      }
      ...
      for (const [channelKey, channelConfig] of Object.entries(
        ...
        try {
          ...
          if (existingChannel && !overwrite) {
            ...
            createdElements.channels[channelKey] = {
              name: channelConfig.name,
              id: existingChannel.id,
              status: "existing",
            };
            console.log(
              `Channel ${channelConfig.name} already exists, using existing channel`,
            );
          } else {
            if (existingChannel && overwrite) {
              await existingChannel.delete(
                "Recreating channel during bot setup",
              );
              console.log(
                `Deleted existing channel ${channelConfig.name} for recreation`,
              );
            }
            ...
            console.log(
              `Created channel: ${channelConfig.name} (${channel.id})`,
            );
          }
        } catch (error) {
          console.error(`Error creating channel ${channelConfig.name}:`, error);
          createdElements.errors.push(
            `Failed to create channel ${channelConfig.name}: ${error.message}`,
          );
        }
      }
      ...
      if (config.channels.support_channel_id) {
        try {
          const supportChannel = guild.channels.cache.get(
          ...
          if (supportChannel) {
            ...
            const ticketEmbed = new EmbedBuilder()
              ...
              .setDescription(
                "Welcome to our support system! Click one of the buttons below to open a support ticket.",
              )
              .addFields(
                {
                  name: "🛠️ Technical Support",
                  value:
                    "Need help with installation, setup, or technical issues? Get assistance from our technical team.",
                  inline: false,
                },
                {
                  name: "🔧 Product Support",
                  value:
                    "Questions about your MicroCoaster™ product, parts, or general usage? Our product experts are here to help.",
                  inline: false,
                },
                {
                  name: "💼 Business Inquiry",
                  value:
                    "Interested in bulk orders, partnerships, or custom solutions? Connect with our business team.",
                  inline: false,
                },
                {
                  name: "👥 Join Our Team",
                  value:
                    "Interested in joining the MicroCoaster™ team? Open a ticket to discuss opportunities.",
                  inline: false,
                },
              )
              .setFooter({
                text: "Your ticket will be handled by our support team as soon as possible.",
              })
              .setTimestamp();
          ...
          createdElements.errors.push(
            `Failed to set up ticket system: ${error.message}`,
          );
        }
      }
      ...
      const summaryEmbed = new EmbedBuilder()
        ...
        .setDescription(
          "The bot has been successfully configured on your server.",
        )
        .setTimestamp();
      ...
      if (Object.keys(createdElements.roles).length > 0) {
        const rolesText = Object.values(createdElements.roles)
          .map(
            (role) => `${role.status === "created" ? "🆕" : "♻️"} ${role.name}`,
          )
          .join("\n");
      ...
      summaryEmbed.addFields(
        {
          name: "🎫 Ticket System",
          value: "Configured in the support channel",
          inline: false,
        },
        {
          name: "⚙️ Configuration",
          value: "All IDs saved to config.json",
          inline: false,
        },
      );
      ...
      if (createdElements.errors.length > 0) {
        ...
        summaryEmbed.addFields({
          ...
          value:
            errorsText +
            (createdElements.errors.length > 5 ? "\n... and more" : ""),
          inline: false,
        });
      }
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setDescription(
          "A critical error occurred during bot setup. Please check bot permissions and try again.",
        )
        .addFields({
          name: "Error Details",
