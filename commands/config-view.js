  ...
  async execute(interaction) {
    try {
      ...
      for (const field of roleFields) {
        const roleId = config.roles ? config.roles[field.key] : config[field.key];
      ...
      for (const field of channelFields) {
        const channelId = config.channels ? config.channels[field.key] : config[field.key];
      ...
      for (const field of categoryFields) {
        const categoryId = config.categories ? config.categories[field.key] : config[field.key];
      ...
      const botValue = `**Prefix:** ${botConfig.prefix || config.bot_prefix || '/'}\n` +
      ...
      const warrantyValue = `**Default Duration:** ${warrantyConfig.default_duration_months || config.default_warranty_duration_months || 12} months\n` +
      ...
      const ticketValue = `**Auto Close:** ${ticketsConfig.auto_close_after_hours || config.ticket_auto_close_after_hours || 24} hours\n` +
      ...
      configEmbed.addFields({
        ...
        value: `**Configured Items:** ${configStats.configured}/${configStats.total}\n` +
...
async function getConfigurationStats(config) {
  const configFields = [
    'premium_role_id', 'warranty_role_id', 'admin_role_id', 'support_team_role_id',
    'technical_team_role_id', 'business_team_role_id', 'recruitment_team_role_id',
    'warranty_activation_channel_id', 'support_channel_id', 'logs_moderation_channel_id',
    'bot_stats_channel_id', 'role_logs_channel_id', 'technical_category_id',
    'product_category_id', 'business_category_id', 'recruitment_category_id'
  ];
