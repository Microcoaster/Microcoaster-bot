  ...
  execute: async (interaction, params = []) => {
    try {
      ...
      const priorityEmbed = new EmbedBuilder()
        ...
        .setDescription(`Select the priority level for ticket **${ticket.ticket_id}**:`)
        .addFields(
      ...
      if (interaction.deferred) {
        await interaction.editReply({ content: '❌ An error occurred while displaying priority menu.' });
      } else {
        await interaction.reply({ content: '❌ An error occurred while displaying priority menu.', flags: MessageFlags.Ephemeral });
      }
    }
  }
};
