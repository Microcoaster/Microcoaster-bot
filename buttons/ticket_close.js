    ...
    try {
      ...
      const confirmEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '⚠️ Warning', value: 'This action cannot be undone. The ticket will be marked as closed and **the channel will be deleted instantly**.', inline: false }
        )
      ...
      if (interaction.deferred) {
        await interaction.editReply({ content: '❌ An error occurred while preparing ticket closure.' });
      } else {
        await interaction.reply({ content: '❌ An error occurred while preparing ticket closure.', flags: MessageFlags.Ephemeral });
      }
    }
  }
};
