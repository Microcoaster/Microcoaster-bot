...
module.exports = {
  execute: async (interaction, params = []) => {
    try {
      ...
      console.log(`\x1b[38;5;4m🎫 Création ticket ${ticketType} pour ${user.username} (${user.id})\x1b[0m`);

      if (!ticketConfig) {
        console.error(`\x1b[38;5;1m❌ Type de ticket invalide: ${ticketType}\x1b[0m`);
      ...
      const existingTicket = await ticketDAO.getUserActiveTicket(user.id, guild.id, ticketType);
      
      if (existingTicket) {
        console.log(`\x1b[38;5;3m⚠️ Ticket existant trouvé pour ${user.username}: ${existingTicket.ticket_id}\x1b[0m`);
        const channel = guild.channels.cache.get(existingTicket.channel_id);
        const embed = new EmbedBuilder()
          .setColor('#ff9900')
          .setTitle('⚠️ Ticket Already Exists')
          .setDescription(`You already have an active ${ticketConfig.name.toLowerCase()} ticket.`)
          .addFields(
            { name: 'Existing Ticket', value: channel ? `${channel}` : `ticket-${existingTicket.ticket_number}`, inline: false }
          )
          .setTimestamp();
      ...
      const channelName = `${ticketConfig.emoji}-${ticketNumber}-${user.username}`.toLowerCase();
      ...
      await ticketDAO.createTicket(user.id, ticketChannel.id, ticketType, 'medium', ticketNumber);

      // Créer l'embed de bienvenue
      const welcomeEmbed = new EmbedBuilder()
        .setColor(ticketConfig.color)
        .setTitle(`${ticketConfig.emoji} ${ticketConfig.name} Ticket #${ticketNumber}`)
        .setDescription(`Hello ${user}! Thank you for creating a ${ticketConfig.name.toLowerCase()} ticket.
...
*This ticket will be handled by our support team shortly.*`)
        .addFields(
          { name: 'Ticket Type', value: ticketConfig.description, inline: true },
      ...
      const ticketManagementButtons = new ActionRowBuilder()
        .addComponents(
          new ButtonBuilder()
      ...
      const managementEmbed = new EmbedBuilder()
        ...
        .addFields(
          { name: '📌 Priority', value: 'Set priority level for this ticket', inline: true },
          { name: '👤 Assign', value: 'Assign this ticket to a staff member', inline: true },
          { name: '🔒 Close', value: 'Close this ticket when resolved', inline: false }
        )
        .setFooter({ text: 'Only staff members can use these controls' })
        .setTimestamp();

      await ticketChannel.send({ embeds: [managementEmbed], components: [ticketManagementButtons] });

      // Ping l'équipe appropriée selon le type de ticket
      const teamRoleId = config.roles[ticketConfig.roleKey] || config.roles.support_team_role_id;
      if (teamRoleId) {
      ...
      const confirmEmbed = new EmbedBuilder()
        ...
        .setDescription(`Your ${ticketConfig.name.toLowerCase()} ticket has been created!`)
        .addFields(
