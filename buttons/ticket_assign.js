    ...
    try {
      ...
      const assignEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Current Status', value: ticket.status || 'Open', inline: true },
          { name: 'Currently Assigned', value: ticket.assigned_to ? `<@${ticket.assigned_to}>` : 'Unassigned', inline: true }
        )
        .setFooter({ text: 'The selected member will be notified and added to this ticket.' })
        .setTimestamp();
      ...
      if (interaction.deferred) {
        await interaction.editReply({ content: '❌ An error occurred while preparing ticket assignment.' });
      } else {
        await interaction.reply({ content: '❌ An error occurred while preparing ticket assignment.', flags: MessageFlags.Ephemeral });
      }
    }
  }
};
